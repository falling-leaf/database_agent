...
from morphingdb_test.image_test.imagenet.import_dataset import (
...
IMAGE_TEST_FILE = 'result/imagenet_test.json'
IMAGE_VECTOR_TEST_FILE = 'result/imagenet_vector_test.json'
timing_data_image = []
timing_data_vector = []
...
def imagenet_test(limit_flag:str, symbol:str = 'cpu'):
    for count in IMAGE_COUNT_LIST:
        ...
        start = time.time()
        sql = "select predict_batch_float8('defect_vec', '{}', image_vector) over (rows between current row and 31 following) from ".format(symbol) + IMAGE_VECTOR_TABLE + " limit {};".format(count)
        cur.execute(sql)
        end = time.time()
