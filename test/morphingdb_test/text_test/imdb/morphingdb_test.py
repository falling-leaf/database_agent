...
import re
import os
from morphingdb_test.config import db_config, imdb_model_path
from morphingdb_test.text_test.imdb.import_dataset import (
    import_imdb_dataset, 
...
TEXT_TEST_FILE = 'result/imdb_test.json'
TEXT_VECTOR_TEST_FILE = 'result/imdb_vector_test.json'
timing_data_image = []
timing_data_vector = []
...
def create_model():
    ...
    res = cur.fetchall()
    if len(res) == 0:
        cur.execute("select create_model('sst2_vec', '{}', '', '');".format(imdb_model_path))
...
def imdb_vector_test(limit_flag:str, symbol:str = 'cpu'):
...
def imdb_all_test():
    ...
    print('imdb vector test done')


if __name__ == "__main__":
    create_model()
    imdb_vector_test('mem_2', 'cpu')